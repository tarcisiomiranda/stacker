...
Use this document as the canonical contract whenever creating or changing `devtui.yml`. The application rejects unknown fields, unsupported versions, empty commands, invalid timeouts, and working directories that do not exist.
...
- `command`: required, non-empty string executed with `sh -lc`. Prefer a project task such as `mise run back:dev` instead of duplicating a long command.
- `cwd`: optional directory. Relative paths are resolved from the directory containing `devtui.yml`. The directory must already exist. Omission means `.`.
...
4. Enable `autostart` only for services that should start whenever DevTUI opens.
5. Keep `version: 1` and remove all undocumented fields.
6. Run `mise run test` and `mise run build` after changing application code. For configuration-only changes, launch `./bin/devtui -config devtui.yml` and confirm the processes start and stop correctly.
